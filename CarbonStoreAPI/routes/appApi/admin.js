...
WHERE
  ...
  GetAppUserCarbonTransaction(res, uid) {
    let sql = `SELECT id,beforeUser,(SELECT userName FROM app_user_list WHERE status = 1 AND uid = tarde.beforeUser) AS beforeUserName,(SELECT userAvatar FROM app_user_list WHERE status = 1 AND uid = tarde.beforeUser) AS beforeUserAvatar,afterUser,(SELECT userName FROM app_user_list WHERE status = 1 AND uid = tarde.afterUser) AS afterUserName,(SELECT userAvatar FROM app_user_list WHERE status = 1 AND uid = tarde.afterUser) AS afterUserAvatar,editorTime,beforeData,afterData,operateData,(SELECT name FROM app_carbon_trade_manner WHERE status = 1 AND id = tarde.operateType) AS operateType,(SELECT title FROM app_carbon_trade_manner WHERE status = 1 AND id = tarde.operateType) AS operateTypeName,(SELECT name FROM app_carbon_trade_type WHERE status = 1 AND id = tarde.operateType) AS operate,(SELECT title FROM app_carbon_trade_type WHERE status = 1 AND id = tarde.operateType) AS operateName FROM app_carbon_tarde AS tarde WHERE beforeUser = ${uid} OR afterUser = ${uid} ORDER BY tarde.editorTime DESC;`;
    db.query(sql, (err, result) => {
      if (err) {
        res.send({ code: err.code, message: err.message.split(":")[0] });
        return;
